...
<section class="content">
	<div class="content_area">
		<h2 class="title01">영화관정보</h2>
		<p>현재위치를 기준으로 근처 영화관을 검색할 수 있습니다.</p>
		...
		<script type="text/javascript">
			...
			function onSuccess(data) {
				...
				var oIcon2 = new nhn.api.map.Icon(
						'http://static.naver.com/maps2/icons/pin_spot2.png', oSize2,
						oOffset2);
		
				for (var i = 0; i < filteredItems.length; i++) {
		
					output += "<li class='MapList'>"
							+ "<span class='tit'>"+filteredItems[i].title + "</span>"
							+ "<span class='addr'>" +filteredItems[i].roadAddress + "</span></li>";
		
					var items_title = filteredItems[i].title;
		
					arrMarker[i] = new nhn.api.map.Marker(oIcon2, {
						title : items_title
					}); //마커 생성
					arrMarker[i].setPoint(new nhn.api.map.TM128(filteredItems[i].mapx,
							filteredItems[i].mapy).toLatLng()); //마커 표시할 좌표 선택
								...
								function() {
									...
									mapInfoTestWindow
											.setContent('<DIV style="border-top:1px solid; border-bottom:2px groove black; border-left:1px solid; border-right:2px groove black;margin-bottom:1px;color:black;background-color:white; width:auto; height:auto;">'
													...
													+ '<span></div>');
									mapInfoTestWindow.setPoint(arrMarker[index]
											.getPoint());
		...
		<form id="searchform" method="post" action="">
			<input type="hidden" name="display" value="6" /> <input type="hidden" name="start" value="1" />
			<div id="search_wrap" class="map_search_wrap">
				<select name="sido" class="input-select">
				...
				<select name="theater" class="input-select">
				...
				<input type="button" value="검색" id="btnsend" class="btn01" />
			</div>
		</form>
		<div class="theater_area clearfix">
			<div class="result_area">
				<div id="theater" class="theater_result">
					<ul class="clear list"><li class="empty_list">검색된 리스트가 없습니다. 검색을 해주세요.</li></ul>
				</div>
				<div class="result_paging" style="display:none;">
			...
			<div class="map_area">
				<div id="demo"></div>
				<div id="testMap" style="border: 1px solid #000; width: 800px; height: 500px; margin: 20px;"></div>					
			</div>
		</div>
		...
		<script type="text/javascript">
			...
			function showPosition(position) {
				...
				var oIcon = new nhn.api.map.Icon('/movie_srcap/theaterMap/brown.png',
						oSize, oOffset);
